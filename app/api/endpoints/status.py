...
def create_status(payload: DeviceStatusCreate, db: Session = Depends(get_db)):
    status_obj = DeviceStatus(**payload.dict())
    db.add(status_obj)
    db.commit()
...
def get_latest_status(device_id: str, db: Session = Depends(get_db)):
    status_obj = (
